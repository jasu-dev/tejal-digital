<section class="py-24">
    <div class="max-w-screen-2xl mx-auto px-8">
        <div class="text-center mb-16" data-aos="fade-up" data-aos-delay="100" data-aos-duration="800">
            <div class="inline-flex items-center gap-2 px-4 py-2 mb-6 gradient-primary rounded-full text-white">
                <span class="text-sm font-medium">How We Work</span>
            </div>
            <h2 class="text-4xl lg:text-5xl font-bold text-white">
                Our Clear Working Process
            </h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            <!-- Connector Lines (Desktop Only) -->
            <div class="hidden md:block absolute top-12 left-0 w-full h-0.5 bg-outline-variant/20 z-0"></div>
            <!-- Step 1 -->
            <div class="relative z-10 group" data-aos="fade-up" data-aos-delay="100" data-aos-duration="800">
                <div
                    class="w-24 h-24 border border-border bg-card rounded-full flex items-center justify-center mb-8 group-hover:border-primary transition-all duration-500 mx-auto">
                    <x-icons.deep-search class="w-8 h-8 text-primary" />
                </div>
                <div class="text-center">
                    <div class="text-primary font-label text-xs uppercase tracking-[0.2em] mb-3">01. Discovery
                    </div>
                    <p class="text-muted-foreground text-sm leading-relaxed max-w-[240px] mx-auto">Deep dive
                        into your business goals, target audience, and market competition.</p>
                </div>
            </div>
            <!-- Step 2 -->
            <div class="relative z-10 group" data-aos="fade-up" data-aos-delay="200" data-aos-duration="800">
                <div
                    class="w-24 h-24 border border-border bg-card rounded-full flex items-center justify-center mb-8 group-hover:border-primary transition-all duration-500 mx-auto">
                    <x-icons.design class="w-8 h-8 text-primary" />
                </div>
                <div class="text-center">
                    <div class="text-primary font-label text-xs uppercase tracking-[0.2em] mb-3">02. Design
                    </div>
                    <p class="text-muted-foreground text-sm leading-relaxed max-w-[240px] mx-auto">Crafting
                        cinematic interfaces that resonate with your brand's core values.</p>
                </div>
            </div>
            <!-- Step 3 -->
            <div class="relative z-10 group" data-aos="fade-up" data-aos-delay="300" data-aos-duration="800">
                <div
                    class="w-24 h-24 border border-border bg-card rounded-full flex items-center justify-center mb-8 group-hover:border-primary transition-all duration-500 mx-auto">
                    <x-icons.code class="w-8 h-8 text-primary" />
                </div>
                <div class="text-center">
                    <div class="text-primary font-label text-xs uppercase tracking-[0.2em] mb-3">03.
                        Development</div>
                    <p class="text-muted-foreground text-sm leading-relaxed max-w-[240px] mx-auto">
                        High-performance engineering using modern serverless stacks and frameworks.</p>
                </div>
            </div>
            <!-- Step 4 -->
            <div class="relative z-10 group" data-aos="fade-up" data-aos-delay="400" data-aos-duration="800">
                <div
                    class="w-24 h-24 border border-border bg-card rounded-full flex items-center justify-center mb-8 group-hover:border-primary transition-all duration-500 mx-auto">
                    <x-icons.launch class="w-8 h-8 text-primary" />
                </div>
                <div class="text-center">
                    <div class="text-primary font-label text-xs uppercase tracking-[0.2em] mb-3">04. Deployment
                    </div>
                    <p class="text-muted-foreground text-sm leading-relaxed max-w-[240px] mx-auto">Strategic
                        roll-out with performance monitoring and continuous optimization.</p>
                </div>
            </div>
        </div>
    </div>
</section>
